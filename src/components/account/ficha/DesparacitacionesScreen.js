...
import { addDesparacitacion, cargarDesparacitaciones, deleteDesparacitacion, editDesparacitacion } from '../../../actions/ficha';
...
export const DesparacitacionesScreen = () => {
  ...
  const { ficha: { desparacitaciones, idFicha }, auth: { rol } } = useSelector( state => state )
  ...
  const handleOpenModal = () => {
    setModalDesparacitacion(true)
  }

  const handleCloseModal = () => {
    setDesparacitacionId('')
    reset();
  ...
  const handleDelete = ( id ) => {
    ...
    Swal.fire({
      title: 'Estás segura/o de eliminar esta desparacitacion?',
      ...
      <div className="my-2 flex sm:flex-row flex-col">
        <div className="w-full flex justify-between">
          <div className="flex space-x-4">
            { (rol === 'VETERINARIO' || rol === 'ADMINISTRADOR') && (
              <button
                ...
                Nueva desparacitacion
              </button>
            )}
            <button
              className="focus:outline-none"
              onClick={ handleRefreshDesparacitaciones }>
                ...
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-headline">
                      ...
                      {
                        desparacitacionId === '' ? 'Agregar desparacitacion' : 'Editar desparacitacion'
                      }
                    </h3>
                    <form onSubmit={ handleAddDesparacitacion }>
                      ...
                      <div className="py-5 flex justify-end items-end">
                        ...
                        <span className="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                          <button type="submit" className="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-primary text-base leading-6 font-medium text-white shadow-sm hover:bg-opacity-75 focus:outline-none focus:border-red-700 transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                            {
                              desparacitacionId === '' ? 'Guardar desparacitacion' : 'Actualizar desparacitacion'
                            }
                          </button>
                        </span>
                      </div>
